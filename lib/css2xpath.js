...
var xpath_to_lower         = function(s) {
	...
	regex_css_wrap_pseudo  = /[^\)]\:(first|limit|last|gt|lt|eq|nth)([^\-]|$)/,
    ...
    css_combinators_callback = function(match, operator, axis, func, literal, exclude, offset, orig) {
        ...
        switch (operator) {
            ...
            case '>':
                return '/'+literal;
            case '+':
                return (offset===0?'.':'')+'/following-sibling::*[1]/self::'+literal;
            case '~':
                return '/following-sibling::'+literal;
            case ',':
                if (axis === undefined);
                    axis = './/';
                return '|'+axis+literal;
    ...
    css_pseudo_classes_callback = function(str, name, m1, m2, arg) {
