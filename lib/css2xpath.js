...
var css2xpath = (function(){
    var re      = [
            ...
            /\[\@(.+?)\^=([^\]]+)\]/g, "[starts-with(@$1,$2)]",
            // $= attrib
            /\[\@(.+?)\$=([^\]]+)\]/g, function(s, a, b) { return "[substring(@".concat(a, ",string-length(@", a, ")-(string-length(", b,")-1))=", b, "]"); },
            // != attrib
            /\[\@(.+?)\!=([^\]]+)\]/g, "[not(@$1) or @$1!=$2]",
            ...
            // :has
            /:has-?(sibling|p[^\-]*-sibling|following-sibling|child|parent|before)?\((\x1F*)(.+?[^\x1F])\2\)/g, function(s, a, b, c){
                var prefix = '';
                if (a === 'sibling')
                ...
                if (a === 'before')
                    prefix += 'preceding::';
                else if (a === 'preceding-sibling' || a === 'previous-sibling')
